...
def get_embedding_info(data, categorical_variables=None, max_dim=50):
    '''
    ...
    return {col:(data[col].nunique(),min(max_dim,(data[col].nunique()+ 1) //2)) for col in categorical_variables}
...
def get_embeddings(X_train, y_train, categorical_embedding_info, is_classification, epochs=100, batch_size=256):
    ...
    nnet.fit(x_inputs, y_train.values, batch_size=batch_size, epochs=epochs, validation_split=0.2, verbose=0)#callbacks=[TQDMNotebookCallback()]
